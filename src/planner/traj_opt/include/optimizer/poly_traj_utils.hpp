...
namespace poly_traj
{
    ...
    private:
        ...
        //下面这两个是计算能量函数对于T和c的梯度，参考汪博论文MINCO轨迹的能量函数部分

        /* 
        * @brief: 计算能量函数对时间T的梯度
        * @param: gdT[输出] \frac{\partial K}{\partial T}（能量函数对时间T的偏导数）
        * @说明: 能量函数反映轨迹的平滑性
        * @说明: MINCO轨迹的总能量由多个多项式片段叠加构成
        * @说明: 因此每个时间段T的梯度仅由该段的局部系数决定
        * 
        * 最终表达式为矩阵R(t)的二次形式
        */
        ...
        /* 
        * @brief: 计算能量函数对系数c的梯度
        * @param: gdC[输出] \frac{\partial K}{\partial c}（能量函数对多项式系数c的偏导数）
        * @说明: 能量函数反映轨迹的平滑性
        * @说明: MINCO轨迹的总能量由多个多项式片段叠加构成
        * @说明: 因此每个系数c的梯度仅由该段的局部系数决定
        */
    ...
    public:
                             ...
                             const Eigen::VectorXd &ts)
        {
            ...
            else
            {
                //T的多次幂
                T1 = ts;
                T2 = T1.cwiseProduct(T1);//T的平方
                T3 = T2.cwiseProduct(T1);//T的三次方
                T4 = T2.cwiseProduct(T2);//T的四次方
                T5 = T4.cwiseProduct(T1);//T的五次方
        ...
        inline void getGrad2TP(EIGENVEC &gdT,
                               EIGENMAT &gdInPs)
        {
            //gdC 是代价函数对多项式系数C的梯度矩阵，即汪博毕设里的\frac{\partial K}{\partial c}

            //求解M^T*G=gdc以获得G
            solveAdjGradC(gdC);

            //反正就是用gdc(G)求出了w关于T的梯度
            addPropCtoT(gdC, gdT);

            //这里gdC和G似乎是一个东西了，因为做了时间归一化，所以M(T)变成了M(1)，看汪博论文
